    ...
    <q-dialog v-model="voteHolderListModal" :maximized="maximized" transition-show="slide-up" transition-hide="slide-down" persistent>
      ...
      <q-layout view="lHh Lpr lFf" container class="shadow-2 rounded-borders bg-white" style="min-width: 40%;">
        <q-header class="bg-white" elevated>
          <q-toolbar>
            <q-avatar>
              <q-icon name="groups" size="md" color="black" />
            </q-avatar>
            <q-toolbar-title class="text-black">
              {{ $t('vote_holder_list') }} <span class="text-body text-grey-7">({{ fixedTime }})</span>
            </q-toolbar-title>
                        ...
                        <table border="0" width="100%" cellpadding="0" cellspacing="0" align="center">
                          <tr>
                            ...
                            <td width="60" class="text-center">
                              <span v-if="item.agree_yn === 'Y'" class="text-green text-bold">{{ $t('vote_agree') }}</span>
                              <span v-if="item.agree_yn === 'N'" class="text-red text-bold">{{ $t('vote_disagree') }}</span>
                            </td>
                          </tr>
                        </table>
                      </div>
