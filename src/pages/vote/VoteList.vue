  ...
  <q-page class="page-default">
    ...
    <q-pull-to-refresh @refresh="refresher">
      <q-infinite-scroll @load="loadMore" :offset="0" ref="infiniteScroll">
        ...
        <div v-for="item in tokenList" :key="item.seq" style="cursor: pointer;z-index: 1;">
          <q-separator />
          <q-item clickable @click="goDetail(item.seq)">
            ...
            <q-item-section>
              <div class="row">
                ...
                <q-item-label v-if="locale === 'ko-KR'" class="col-12 text-grey-6 text-bold">{{ item.project_title_ko }}</q-item-label>
                <q-item-label v-else class="col-12 text-grey-6 text-bold">{{ item.project_title }}</q-item-label>
                ...
                <div class="q-pt-xs q-pb-xs">
                  <table border="0" width="100%" cellpadding="0" cellspacing="0" align="center">
                    <tr class="text-green">
                      <td width="">
                        <span class="text-bold">{{ $t('vote_agree') }}</span>
                      </td>
                      <td>
                        &nbsp;:&nbsp;&nbsp;
                      </td>
                      <td class="text-right">
                        <span class="text-bold">{{ item.rate_agree.toFixed(3) }} %</span>
                      </td>
                      <td width="19">
                      ...
                      <td class="text-right">
                        <span class="text-black">
                          <span class="text-green text-bold">{{ item.total_agree }}</span> / {{ item.total }} )
                        </span>
                      </td>
                    </tr>
                    <tr class="text-red-7">
                      <td width="">
                        <span class="text-bold">{{ $t('vote_disagree') }}</span>
                      </td>
                      <td>
                        &nbsp;:&nbsp;&nbsp;
                      </td>
                      <td class="text-right">
                        <span class="text-bold">{{ item.rate_disagree.toFixed(3) }} %</span>
                      </td>
                      <td>
                        &nbsp;&nbsp;&nbsp;<span class="text-black">(</span>
                      </td>
                      <td class="text-right">
                        <span class="text-black">
                          <span class="text-red text-bold">{{ item.total_disagree }}</span> / {{ item.total }} )
                        </span>
                      </td>
                    </tr>
                  </table>
                ...
                <q-item-label class="col-12 text-grey-7">{{ item.start_time }} ~ {{ item.end_time }}</q-item-label>
